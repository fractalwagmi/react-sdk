...
import { useFractalItems } from 'hooks/use-fractal-items';
...
describe('useFractalItems', () => {
  ...
  it('returns the expected items', async () => {
    ...
    const { result } = renderHook(() => useFractalItems(), { wrapper });
  ...
  it('only makes one network request until refetch is called', async () => {
    const { rerender, result } = renderHook(() => useFractalItems(), {
      wrapper,
    });
    await act(async () => void {});
